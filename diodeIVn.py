...
class Expt(QWidget):
	...
	def fit_curve(self):
		...
		if f != None:
			self.traces.append(self.pwin.plot(f[2], f[3], pen = self.traceCols[self.trial%5]))
			self.trial += 1
			...
			ss1 = '%5.2e'%Io
			ss2 = '%5.2f'%n
			self.msg(self.tr('Fitted with Diode Equation : Io = ') +ss1 + self.tr(' mA , Ideality factor = ') + ss2 + 'Knee Voltage Approx: ' + str(f[4]))
